...
class InsightsResponseBody(BaseModel):
    ...
    """
    insights: Optional[conlist(InsightResponse)] = None
    pagination: Optional[PaginationResponse] = None
    __properties = ["insights", "pagination"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> InsightsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in insights (list)
        _items = []
        if self.insights:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> InsightsResponseBody:
        """Create an instance of InsightsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return InsightsResponseBody.parse_obj(obj)

        _obj = InsightsResponseBody.parse_obj({
            "insights": [InsightResponse.from_dict(_item) for _item in obj.get("insights")] if obj.get("insights") is not None else None,
