...
class InstitutionsResponseBody(BaseModel):
    ...
    """
    institutions: Optional[conlist(InstitutionResponse)] = None
    pagination: Optional[PaginationResponse] = None
    __properties = ["institutions", "pagination"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> InstitutionsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in institutions (list)
        _items = []
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> InstitutionsResponseBody:
        """Create an instance of InstitutionsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return InstitutionsResponseBody.parse_obj(obj)

        _obj = InstitutionsResponseBody.parse_obj({
            "institutions": [InstitutionResponse.from_dict(_item) for _item in obj.get("institutions")] if obj.get("institutions") is not None else None,
