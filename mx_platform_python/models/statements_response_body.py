...
class StatementsResponseBody(BaseModel):
    """
    StatementsResponseBody
    """
    pagination: Optional[PaginationResponse] = None
    statements: Optional[conlist(StatementResponse)] = None
    __properties = ["pagination", "statements"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> StatementsResponseBody:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in statements (list)
        _items = []
        if self.statements:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> StatementsResponseBody:
        """Create an instance of StatementsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return StatementsResponseBody.parse_obj(obj)

        _obj = StatementsResponseBody.parse_obj({
            "pagination": PaginationResponse.from_dict(obj.get("pagination")) if obj.get("pagination") is not None else None,
            "statements": [StatementResponse.from_dict(_item) for _item in obj.get("statements")] if obj.get("statements") is not None else None
        })
        return _obj
