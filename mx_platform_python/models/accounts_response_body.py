...
class AccountsResponseBody(BaseModel):
    ...
    """
    accounts: Optional[conlist(AccountResponse)] = None
    pagination: Optional[PaginationResponse] = None
    __properties = ["accounts", "pagination"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AccountsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in accounts (list)
        _items = []
        if self.accounts:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AccountsResponseBody:
        """Create an instance of AccountsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AccountsResponseBody.parse_obj(obj)

        _obj = AccountsResponseBody.parse_obj({
            "accounts": [AccountResponse.from_dict(_item) for _item in obj.get("accounts")] if obj.get("accounts") is not None else None,
