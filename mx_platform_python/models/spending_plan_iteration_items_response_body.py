...
class SpendingPlanIterationItemsResponseBody(BaseModel):
    """
    SpendingPlanIterationItemsResponseBody
    """
    iteration_items: Optional[conlist(SpendingPlanIterationItemResponse)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SpendingPlanIterationItemsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in iteration_items (list)
        _items = []
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SpendingPlanIterationItemsResponseBody:
        """Create an instance of SpendingPlanIterationItemsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SpendingPlanIterationItemsResponseBody.parse_obj(obj)

        _obj = SpendingPlanIterationItemsResponseBody.parse_obj({
            "iteration_items": [SpendingPlanIterationItemResponse.from_dict(_item) for _item in obj.get("iteration_items")] if obj.get("iteration_items") is not None else None,
