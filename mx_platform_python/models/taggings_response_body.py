...
class TaggingsResponseBody(BaseModel):
    """
    TaggingsResponseBody
    """
    pagination: Optional[PaginationResponse] = None
    taggings: Optional[conlist(TaggingResponse)] = None
    __properties = ["pagination", "taggings"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TaggingsResponseBody:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in taggings (list)
        _items = []
        if self.taggings:
            for _item in self.taggings:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TaggingsResponseBody:
        """Create an instance of TaggingsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TaggingsResponseBody.parse_obj(obj)

        _obj = TaggingsResponseBody.parse_obj({
            "pagination": PaginationResponse.from_dict(obj.get("pagination")) if obj.get("pagination") is not None else None,
            "taggings": [TaggingResponse.from_dict(_item) for _item in obj.get("taggings")] if obj.get("taggings") is not None else None
        })
        return _obj
