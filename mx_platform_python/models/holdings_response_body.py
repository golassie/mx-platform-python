...
class HoldingsResponseBody(BaseModel):
    ...
    """
    holdings: Optional[conlist(HoldingResponse)] = None
    pagination: Optional[PaginationResponse] = None
    __properties = ["holdings", "pagination"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> HoldingsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in holdings (list)
        _items = []
        if self.holdings:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> HoldingsResponseBody:
        """Create an instance of HoldingsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HoldingsResponseBody.parse_obj(obj)

        _obj = HoldingsResponseBody.parse_obj({
            "holdings": [HoldingResponse.from_dict(_item) for _item in obj.get("holdings")] if obj.get("holdings") is not None else None,
