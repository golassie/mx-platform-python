...
class RewardsResponseBody(BaseModel):
    ...
    RewardsResponseBody
    """
    rewards: Optional[conlist(RewardsResponse)] = None
    pagination: Optional[PaginationResponse] = None
    __properties = ["rewards", "pagination"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RewardsResponseBody:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic.v1 by calling `to_dict()` of each item in rewards (list)
        _items = []
        if self.rewards:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RewardsResponseBody:
        """Create an instance of RewardsResponseBody from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RewardsResponseBody.parse_obj(obj)

        _obj = RewardsResponseBody.parse_obj({
            "rewards": [RewardsResponse.from_dict(_item) for _item in obj.get("rewards")] if obj.get("rewards") is not None else None,
